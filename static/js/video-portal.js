...
var statusCheckForUserAgreement = document.getElementById(
  'proceed-data-not-checked'
)
  ...
  db.collection('interview-schedule')
    ...
    .then(function (querySnapshot) {
      if (querySnapshot.size > 0) {
        ...
        console.log('Interview Code matches')
        setTimeout(function () {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  proceedButton.addEventListener('click', function () {
    if (agreeCheckbox.checked) {
      statusCheckForUserAgreement.textContent = ''
      setTimeout(function () {
      ...
      statusCheckForUserAgreement.textContent =
        'Please accept the agreements to proceed!'
    }
  })
})
      ...
      .then((querySnapshot) => {
        ...
        if (!querySnapshot.empty) {
          ...
          db.collection('candidates')
            ...
            .then(() => {
              storeDatabaseResponse.textContent = 'Data Stored!'
              setTimeout(function () {
                userDataSection.style.display = 'none'

                videoSection.style.display = 'block'
                openVideoPage(interviewCodeInput.value, candidateEmail)
              }, 1000)
...
function openVideoPage(interviewCode, candidateEmail) {
  fetch(
    `/video_page?interviewCode=${interviewCode}&candidateEmail=${candidateEmail}`
  )
    .then((response) => {
      if (response.ok) {
        window.location.href = response.url
      } else {
        console.error('Failed to load video_page:', response)
      }
    })
    .catch((error) => {
      console.error('Error fetching video_page:', error)
    })
}
...
function displayQuestions() {
  ...
  var interviewStatus = document.getElementById('interview-status')
